...
// sets initial node with values and appends to beginning of list
void setBeginningNode(GameInfo *&pHead, GameInfo gameInfo) {
  ...
  pTemp->moveNumber = gameInfo.moveNumber;
  pTemp->pNext = pHead;
  pHead = pTemp;
}

// prepends new nodes to list
void prependNode(GameInfo *&pHead, GameInfo gameInfo) {
  GameInfo *pTemp = new GameInfo;
  pTemp->wumpusRoom = gameInfo.wumpusRoom;
  pTemp->personRoom = gameInfo.personRoom;
  pTemp->batsRoom1 = gameInfo.batsRoom1;
...
// deletes the current pHead node and points to the next
void undoMove(GameInfo *&pHead) {
  GameInfo *pCurrent = pHead;
  pHead = pHead->pNext;
  delete pCurrent;
}

// updates class members with the next node that pHead points to
void restorePrevData(GameInfo *&pHead, GameInfo *gameInfo) {
  gameInfo->wumpusRoom = pHead->wumpusRoom;
  gameInfo->personRoom = pHead->personRoom;
  gameInfo->batsRoom1 = pHead->batsRoom1;
  gameInfo->batsRoom2 = pHead->batsRoom2;
  gameInfo->pitRoom1 = pHead->pitRoom1;
  gameInfo->pitRoom2 = pHead->pitRoom2;
  gameInfo->arrowRoom = pHead->arrowRoom;
}
//--------------------------------------------------------------------------------
...
int main()
{
    ...
    GameInfo *pHead = NULL;
    setBeginningNode(pHead, gameInfo); //populates first node data members with the initial values and prepends to list
        ...
        while ( true) {
            ...
            if( typeOfMove == 'D') {
              ...
              if(gameInfo.moveNumber == 1) {
                ...
                undoMove(pHead);
                restorePrevData(pHead, &gameInfo);
                gameInfo.moveNumber--;
                continue;
              }
            }
